...
func Process(response *Response, serviceID, serviceName, serviceVersion string, m *Metrics) {
	for _, d := range response.Data {
		for datacenter, stats := range d.Datacenter {
			...
			m.ObjectStoreClassAOperationsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ObjectStoreClassAOperations))
			m.ObjectStoreClassBOperationsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ObjectStoreClassBOperations))
			m.ObjectStoreReadRequestsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ObjectStoreReadRequests))
			m.ObjectStoreWriteRequestsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ObjectStoreWriteRequests))
