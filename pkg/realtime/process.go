...
func process(serviceID, serviceName, datacenter string, stats Datacenter, m *Metrics) {
	...
	m.ComputeExecutionTimeTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeExecutionTimeMilliseconds) / 1000.0)
	...
	m.ComputeHandoffTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeHandoff))
	...
	m.ComputePlatformInternalErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputePlatformInternalErrors))
	m.ComputePlatformInvalidRequestErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputePlatformInvalidRequestErrors))
	...
	m.ComputeRequestTimeBilledTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeRequestTimeBilledMilliseconds) / 1000.0)
	m.ComputeRequestTimeTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeRequestTimeMilliseconds) / 1000.0)
	...
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "103").Add(float64(stats.ComputeRespStatus103))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "200").Add(float64(stats.ComputeRespStatus200))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "204").Add(float64(stats.ComputeRespStatus204))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "206").Add(float64(stats.ComputeRespStatus206))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "301").Add(float64(stats.ComputeRespStatus301))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "302").Add(float64(stats.ComputeRespStatus302))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "304").Add(float64(stats.ComputeRespStatus304))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "400").Add(float64(stats.ComputeRespStatus400))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "401").Add(float64(stats.ComputeRespStatus401))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "403").Add(float64(stats.ComputeRespStatus403))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "404").Add(float64(stats.ComputeRespStatus404))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "416").Add(float64(stats.ComputeRespStatus416))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "429").Add(float64(stats.ComputeRespStatus429))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "500").Add(float64(stats.ComputeRespStatus500))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "501").Add(float64(stats.ComputeRespStatus501))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "502").Add(float64(stats.ComputeRespStatus502))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "503").Add(float64(stats.ComputeRespStatus503))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "504").Add(float64(stats.ComputeRespStatus504))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "505").Add(float64(stats.ComputeRespStatus505))
	m.ComputeStatusCodeTotal.WithLabelValues(serviceID, serviceName, datacenter, "530").Add(float64(stats.ComputeRespStatus530))
	m.ComputeRespStatus103Total.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeRespStatus103))
	m.ComputeRuntimeErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeRuntimeErrorsTotal))
	m.ComputeSandboxesTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeSandboxes))
	m.ComputeServiceBackendReq5xxErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReq5xxErrorsTotal))
	m.ComputeServiceBackendReqConnErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqConnErrorsTotal))
	m.ComputeServiceBackendReqConnOtherErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqConnOtherErrorsTotal))
	m.ComputeServiceBackendReqConnRefusedErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqConnRefusedErrorsTotal))
	m.ComputeServiceBackendReqConnTimeoutErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqConnTimeoutErrorsTotal))
	m.ComputeServiceBackendReqDNSErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqDNSErrorsTotal))
	m.ComputeServiceBackendReqErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqErrorsTotal))
	m.ComputeServiceBackendReqHTTPErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqHTTPErrorsTotal))
	m.ComputeServiceBackendReqHTTPIncompleteErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqHTTPIncompleteErrorsTotal))
	m.ComputeServiceBackendReqHTTPOtherErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqHTTPOtherErrorsTotal))
	m.ComputeServiceBackendReqHTTPProtoV1ErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqHTTPProtoV1ErrorsTotal))
	m.ComputeServiceBackendReqHTTPProtoV2ErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqHTTPProtoV2ErrorsTotal))
	m.ComputeServiceBackendReqHTTPTimeoutErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqHTTPTimeoutErrorsTotal))
	m.ComputeServiceBackendReqOtherErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqOtherErrorsTotal))
	m.ComputeServiceBackendReqTLSErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqTLSErrorsTotal))
	m.ComputeServiceBackendReqTLSOtherErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqTLSOtherErrorsTotal))
	m.ComputeServiceBackendReqTLSServerCertErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceBackendReqTLSServerCertErrorsTotal))
	m.ComputeServiceChainErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceChainErrors))
	m.ComputeServiceLimitsErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceLimitsErrors))
	m.ComputeServiceMemoryExceededErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceMemoryExceededErrors))
	m.ComputeServiceResourceLimitsErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceResourceLimitsErrors))
	m.ComputeServiceRuntimeErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceRuntimeErrors))
	m.ComputeServiceTimeoutErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceTimeoutErrors))
	m.ComputeServiceVCPUExceededErrorsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ComputeServiceVCPUExceededErrors))
	...
	m.ImgOptoAVIFCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoAVIFCount))
	m.ImgOptoComputeRequestsTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoComputeRequests))
	m.ImgOptoGIFCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoGIFCount))
	m.ImgOptoJPEGCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoJPEGCount))
	m.ImgOptoJPEGXLCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoJPEGXLCount))
	m.ImgOptoMP4CountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoMP4Count))
	m.ImgOptoPNGCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoPNGCount))
	...
	m.ImgOptoSVGCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoSVGCount))
	...
	m.ImgOptoWebPCountTotal.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.ImgOptoWebPCount))
