...
func process(serviceID, serviceName, datacenter string, stats Datacenter, m *Metrics) {
	...
	m.DDOSProtectionRequestsAllowCount.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.DDOSProtectionRequestsAllowCount))
	m.DDOSProtectionRequestsDetectCount.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.DDOSProtectionRequestsDetectCount))
	m.DDOSProtectionRequestsMitigateCount.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.DDOSProtectionRequestsMitigateCount))
	...
	m.RequestCollapseUnusableCount.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.RequestCollapseUnusableCount))
	m.RequestCollapseUsableCount.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.RequestCollapseUsableCount))
